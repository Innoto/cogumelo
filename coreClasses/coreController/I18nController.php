...
class I18nController {
    ...
    */
	function setLang() {
	    ...
	    bindtextdomain($this->textdomain, I18N);
    ...
    */
	function c_i18n_gettext(){
    	...
    	foreach ($this->dir_lc as $l){
	        ...
	        if (is_dir($l)){
	          ...
	          while ($file = readdir($handle)) {
	            if ($file=='cogumelo_php.po'){
	              $existPHP = true;
	            }
	            if ($file=='cogumelo_tpl.po'){
	              $existTPL = true;
	            }
	            if ($file=='cogumelo_js.po'){
	              $existJS = true;
	            }
	    ...
	    foreach ($this->dir_lc as $l){
	      if ($existPHP){ //merge
		      ...
		      $oldEntries = Gettext\Extractors\Po::extract($l.'/'.$this->textdomain.'_php.po');
	      ...
	      Gettext\Generators\Po::generateFile($entries, $l.'/'.$this->textdomain.'_php.po');
	    }
        ...
        foreach ($this->dir_lc as $l){
	        ...
	        //Get the translations of the code that are stored in a po file
	        $oldEntries = Gettext\Extractors\Po::extract($l.'/'.$this->textdomain.'_js.po');
          ...
          Gettext\Generators\Po::generateFile($entries, $l.'/'.$this->textdomain.'_js.po');
        }
	...
	function c_i18n_compile() {
		foreach ($this->dir_lc as $l){
	      exec('msgfmt -c -v -o '.$l.'/'.$this->textdomain.'_php.mo '.$l.'/'.$this->textdomain.'_php.po');
	      exec('msgfmt -c -v -o '.$l.'/'.$this->textdomain.'_js.mo '.$l.'/'.$this->textdomain.'_js.po');
	      exec('msgfmt -o '.$l.'/'.$this->textdomain.'_tpl.mo '.$l.'/'.$this->textdomain.'_tpl.po');
	    }
	}
