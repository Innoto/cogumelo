...
class i18nScriptController {
  ...
  function msgCatFacade( $param ) {
    // Cogumelo::log( 'cgml-msgcat.sh '.$param, 'msgCatFacade' );
    // echo( "\n - cgml-msgcat.sh ".$param."\n\n" );
    exec( $this->dir_modules_c.'i18nGetLang/classes/cgml-msgcat.sh '.$param );
  }


  function execFacade( $param ) {
    // Cogumelo::log( 'exec '.$param, 'execFacade' );
    // echo( "\n - exec ".$param."\n\n" );
    exec( $param );
  }
  ...
  function updateModulePo($module){
    ...
    foreach( $this->lang as $l => $lang ) {
      if(file_exists($module.'/'.$this->textdomain.'_'.$l.'_prev.po') && file_exists($module.'/'.$this->textdomain.'_'.$l.'_tpl.po')){
        $this->msgCatFacade( $module.'/'.$this->textdomain.'_'.$l.'_tmp.po '.$module.'/'.$this->textdomain.'_'.$l.'_prev.po '.$module.'/'.$this->textdomain.'_'.$l.'_tpl.po');
      }
      else{
        if(file_exists($module.'/'.$this->textdomain.'_'.$l.'_prev.po')){
          $this->msgCatFacade( $module.'/'.$this->textdomain.'_'.$l.'_tmp.po '.$module.'/'.$this->textdomain.'_'.$l.'_prev.po');
        }
        else{
           if(file_exists($module.'/'.$this->textdomain.'_'.$l.'_tpl.po')){
             $this->msgCatFacade( $module.'/'.$this->textdomain.'_'.$l.'_tmp.po '.$module.'/'.$this->textdomain.'_'.$l.'_tpl.po');
           }
         }
      }
      if(file_exists($module.'/'.$this->textdomain.'_'.$l.'_js.po')){
        if(file_exists($module.'/'.$this->textdomain.'_'.$l.'_tmp.po')){
          $this->msgCatFacade( $module.'/'.$this->textdomain.'_'.$l.'.po '.$module.'/'.$this->textdomain.'_'.$l.'_tmp.po '.$module.'/'.$this->textdomain.'_'.$l.'_js.po');
        }
      }
      else{
        if(file_exists($module.'/'.$this->textdomain.'_'.$l.'_tmp.po')){
          $this->msgCatFacade( $module.'/'.$this->textdomain.'_'.$l.'.po '.$module.'/'.$this->textdomain.'_'.$l.'_tmp.po');
        }
      }
  ...
  function updateAppPo(){
    ...
    foreach( $this->dir_lc as $l ) {
      if(file_exists($l.'/'.$this->textdomain.'_prev.po') && file_exists($l.'/'.$this->textdomain.'_tpl.po')){
        $this->msgCatFacade( $l.'/'.$this->textdomain.'_tmp.po '.$l.'/'.$this->textdomain.'_tpl.po');
      }
      else{
        if(file_exists($l.'/'.$this->textdomain.'_prev.po')){
          $this->msgCatFacade( $l.'/'.$this->textdomain.'_tmp.po '.$l.'/'.$this->textdomain.'_prev.po');
        }
        else{
      ...
      if(file_exists($l.'/'.$this->textdomain.'_js.po')){
        $this->msgCatFacade( $l.'/'.$this->textdomain.'_app.po '.$l.'/'.$this->textdomain.'_tmp.po '.$l.'/'.$this->textdomain.'_js.po');
      }
      else{
        $this->msgCatFacade( $l.'/'.$this->textdomain.'_app.po '.$l.'/'.$this->textdomain.'_tmp.po');
      }
  ...
  **/
  function getSystemPo($system){
    ...
    if ($dh = opendir($path)) {
      ...
      foreach($this->dir_lc as $l=>$lang){
        $this->msgCatFacade( $path.$this->textdomain.'_'.$system.'_'.$l.'.po '.$all[$l]);
        $this->execFacade( 'cp '.$path.'/'.$this->textdomain.'_'.$system.'_'.$l.'.po '.$lang.'/'.$this->textdomain.'_'.$system.'.po');
        $this->execFacade( 'rm '.$path.'/'.$this->textdomain.'_'.$system.'_'.$l.'.po');
      }
    }
  }
  ...
  **/
  function generateModulePo($module, $files, $type){
    if(is_dir($module.'/translations')){
      ...
      $this->execFacade( 'mkdir '.$module.'/translations');
      $module = $module.'/translations';
    }
    foreach( $this->lang as $l => $lang) {
      switch($type){
  ...
  **/
  function generateModuleTplPo($module,$filesTpl){
    ...
    $this->execFacade(  'chmod 700 '.$smartygettext );
    ...
    foreach ($filesTpl as $a){
      ...
      if(count($a_cogumelo_parts)>1){
        ...
        $this->execFacade( 'cp '.$a.' '.Cogumelo::getSetupValue( 'smarty:tmpPath' ).'/'.$cogumelo_name);
      }

      $a_geozzy_parts = explode($this->dir_modules_dist,$a);
      if(count($a_geozzy_parts)>1){
        $geozzy_name = str_replace('/','_',$a_geozzy_parts[1]);
        $this->execFacade( 'cp '.$a.' '.Cogumelo::getSetupValue( 'smarty:tmpPath' ).'/'.$geozzy_name);
      }
    }
    foreach( $this->lang as $l => $lang ) {
      $this->execFacade( $smartygettext.' -o '.$module.'/'.$this->textdomain.'_'.$l.'_tpl.po '.Cogumelo::getSetupValue( 'smarty:tmpPath' ));
    }
  }
  ...
  **/
  function generateAppTplPo($filesTpl){
    ...
    foreach ($filesTpl as $a){
      ...
      $this->execFacade( 'cp '.$a.' '.Cogumelo::getSetupValue( 'smarty:tmpPath' ).'/'.$name);
    }

    foreach( $this->dir_lc as $l ) {
      $this->execFacade( $smartygettext.' -o '.$l.'/'.$this->textdomain.'_tpl.po '.Cogumelo::getSetupValue( 'smarty:tmpPath' ));
    }
  }
  ...
  public function c_i18n_compile() {
    foreach ($this->dir_lc as $l){
      ...
      $this->msgCatFacade( $l.'/'.$this->textdomain.'.po '.$l.'/'.$this->textdomain.'_app.po '.$l.'/'.$this->textdomain.'_cogumelo.po '.$l.'/'.$this->textdomain.'_geozzy.po');
      ...
      $this->execFacade( 'php '.$this->dir_modules_c.'/i18nServer/classes/po2json.php -i '.$l.'/'.$this->textdomain.'.po -o '.$l.'/translation.json');
    }
    ...
    foreach ($this->dir_lc as $l){
      $this->execFacade( 'rm '.$l.'/'.$this->textdomain.'.po');
      $this->execFacade( 'rm '.$l.'/'.$this->textdomain.'_cogumelo.po');
      $this->execFacade( 'rm '.$l.'/'.$this->textdomain.'_geozzy.po');
    }
  }
  ...
  public function c_i18n_json() {
    foreach ($this->dir_lc as $l){
      $this->execFacade( 'php '.$this->dir_modules_c.'/i18nServer/classes/po2json.php -i '.$l.'/'.$this->textdomain.'_js.po -o '.$l.'/translation.json');
    ...
    if ($dh = opendir($this->dir_modules_c)) {
      while (($module = readdir($dh)) !== false) {
        if($this->dir_modules_c.$module!='.' && $this->dir_modules_c.$module!='..'){
          $this->execFacade( 'rm '.$this->dir_modules_c.$module.'/translations/*.po');
        }
      }
    }
    if ($dh = opendir($this->dir_modules_dist)) {
      while (($module = readdir($dh)) !== false) {
        if($this->dir_modules_dist.$module!='.' && $this->dir_modules_c.$module!='..'){
          $this->execFacade( 'rm '.$this->dir_modules_dist.$module.'/translations/*.po');
        }
      }
    }
    foreach( $this->dir_lc as $l ) {
      $this->execFacade( 'rm '.$l.'/*.po');
    }
  }
}
